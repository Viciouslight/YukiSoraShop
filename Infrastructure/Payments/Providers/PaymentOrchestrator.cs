...
namespace Infrastructure.Payments.Providers
{
    public sealed class PaymentOrchestrator : IPaymentOrchestrator
    {
        ...
        private readonly IMoMoGateway _moMoGateway;
        private readonly IUnitOfWork _uow;
        private readonly ILogger<PaymentOrchestrator> _logger;

        public PaymentOrchestrator(IHttpContextAccessor http, IVnPayGateway vnPayGateway, IMoMoGateway moMoGateway, IUnitOfWork uow, IInvoiceService invoiceService, ILogger<PaymentOrchestrator> logger)
        {
            _http = http;
        ...
        public async Task<PaymentCheckoutDTO> CreateMoMoCheckoutAsync(int orderId, string clientIp, string? orderDesc, CancellationToken ct = default)
        {
            _logger.LogInformation("Creating MoMo checkout for order {OrderId}", orderId);
            ...
            try
            {
                var method = await EnsurePaymentMethodAsync("MoMo", "MoMo payment gateway", ct);
                var payment = new Payment
                {
                    OrderId = order.Id,
                    PaymentMethodId = method.Id,
                ...
                var rawIp = _http.HttpContext is null ? clientIp : _http.HttpContext.Connection.RemoteIpAddress?.ToString() ?? clientIp;
                var ip = NormalizeClientIp(rawIp);

                var checkout = await _moMoGateway.GenerateCheckoutUrlAsync(
                    orderId: order.Id,
                    amountVnd: amountVnd,
                    clientIp: ip,
                    orderDesc: orderDesc,
                ...
                await _uow.SaveChangesAsync();
                _logger.LogInformation("Generated MoMo checkout URL for order {OrderId}", order.Id);
                await transaction.CommitAsync();
                return checkout;
            }
            catch
