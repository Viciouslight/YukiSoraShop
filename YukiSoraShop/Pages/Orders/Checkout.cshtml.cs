...
namespace YukiSoraShop.Pages.Orders
{
    ...
    public class OrdersCheckoutModel : PageModel
    {
        ...
        public async Task<IActionResult> OnGetAsync()
        {
            ...
            if (string.Equals(Provider, "VNPay", StringComparison.OrdinalIgnoreCase) && !await IsVnPayActiveAsync())
            {
            ...
            if (string.Equals(Provider, "MoMo", StringComparison.OrdinalIgnoreCase) && !await IsMoMoActiveAsync())
            {
                TempData["Error"] = "Phương thức MoMo hiện không khả dụng. Vui lòng chọn phương thức khác.";
                return RedirectToPage("/Orders/PaymentMethod", new { OrderId });
            }
        ...
        public async Task<IActionResult> OnPostAsync()
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    dto = await _payment.CreateCheckoutAsync(cmd);
            ...
            {
                _logger.LogError(ex, "Checkout OnPost: Error creating checkout for Order {OrderId} provider {Provider}", OrderId, Provider);
        ...
        private async Task<bool> IsMoMoActiveAsync()
        {
            var method = await _uow.PaymentMethodRepository.FindOneAsync(pm => pm.Name == "MoMo");
            return method?.IsActive ?? false;
        }
    }
}
